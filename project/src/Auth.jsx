...
import { 
  ...
  AlertCircle, Loader2, Eye, EyeOff, Check, X, ArrowLeft 
} from 'lucide-react';

const InputField = React.memo(({ name, icon: Icon, placeholder, type = "text", error, showEyeToggle, value, onChange, onBlur, showPassword, setShowPassword, autoComplete, isCompact, autoFocus }) => (
...
export default function Auth({ onGoToLanding }) {
  ...
  // Funkcja walidująca konkretne pole (Walidacja na żywo)
  ...
  // Efekt dla punktu 4: Auto-fill (Ładowanie zapamiętanego e-maila)
  useEffect(() => {
  ...
  const handleChange = useCallback((e) => {
    ...
    setAuthError('');
    // Usuwamy błąd podczas pisania, jeśli użytkownik koryguje dane
    if (errors[name]) {
        setErrors(prev => {
            const newErrs = { ...prev };
            delete newErrs[name];
  ...
  const handleAuth = async (e) => {
    ...
    // Realizacja punktu 4: Zapisywanie e-maila
    if (rememberMe) {
    ...
    try {
      ...
      triggerShake();
      setAuthError(err.message === "Invalid login credentials" ? "BŁĘDNY E-MAIL LUB HASŁO" : err.message.toUpperCase());
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className={`relative w-full max-w-[380px] z-10 ${isShaking ? 'animate-shake' : ''}`}>
        <div className="bg-[#0f172a]/80 backdrop-blur-2xl border border-slate-800 p-6 sm:p-8 rounded-3xl shadow-[0_0_60px_rgba(0,0,0,0.7)] relative z-10">
          ...
          <div className="text-center mb-8">
            <div onClick={onGoToLanding} className="group cursor-pointer inline-block">
          ...
          {isEmailSent ? (
            <div className="text-center py-4 animate-in fade-in zoom-in-95 duration-500">
              ...
              <h2 className="text-white font-bold text-xl mb-3 uppercase tracking-tight">Sprawdź e-mail!</h2>
              <p className="text-slate-400 text-[13px] leading-relaxed mb-8">
                Wysłaliśmy instrukcje na adres:<br/>
            ...
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <form onSubmit={handleResetRequest}>
                ...
                >
                  {loading ? <Loader2 className="animate-spin size-4" /> : <>Wyślij link <ArrowRight size={16} /></>}
                </button>
              </form>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleAuth} noValidate className="space-y-1">
                {isRegistering && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-3 animate-in fade-in slide-in-from-top-2">
                ...
                <div className="relative">
                  <InputField 
                    name="password" 
                    icon={Lock} 
                    placeholder={isRegistering ? "Hasło" : "Hasło użytkownika"} 
              ...
              <div className="grid grid-cols-2 gap-4">
                {['Google', 'Facebook'].map(plat => (
                  <button key={plat} type="button" className="flex items-center justify-center gap-3 py-3 border border-slate-800 rounded-2xl hover:bg-white/[0.03] hover:border-slate-600 transition-all text-[10px] font-black uppercase tracking-widest text-slate-400">
                    {plat === 'Google' ? (
                    ...
                    ) : <span className="text-blue-500 text-sm font-black">f</span>}
                    {plat}
                  </button>
                ))}
              </div>
